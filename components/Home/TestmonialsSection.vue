<template>
  <section class="TestmonialsSection pt-28 relative">
    <div
      class="absolute top-0 left-0 w-full h-[calc(100%-86px)] bg-primary-500"
    ></div>
      ...
      >
        <div class="">
          ...
          <p class="text-2xl font-normal text-white">
            حلول قانونية دقيقة تدعم نجاحك وتحقق أهدافك
          </p>
        </div>
      ...
      <div class="">
        ...
        >
          <SwiperSlide v-for="(value, index) in 10" class="pb-1 px-1">
            <div
              class="Card boxShadow py-10 px-8 flex flex-col gap-4 bg-white rounded-3xl h-fit"
            >
              <div class="flex gap-2">
                <div class="size-12 rounded-full bg-primary-50"></div>
                <div class="flex flex-col">
                  <h2 class="text-text font-bold text-base md:text-lg mb-1">
                    {{ index }}
                  </h2>
                  <div class="flex gap-1.5 items-center">
                    <LucideStar class="text-[#FFB800]" v-for="value in 5" />
                  </div>
                </div>
              </div>
              <p class="text-text text-sm md:text-base font-normal">
                جربتنا مع مكتبكم كانت استثنائية. حصلنا على استشارات دقيقة وسريعة
                ساعدتنا على حل نزاع معقد في وقت قياسي.
              </p>
            </div>
          </SwiperSlide>
        </Swiper>
