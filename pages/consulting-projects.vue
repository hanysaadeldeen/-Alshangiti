<template>
  <main>
    ...
    />
    <WhatTheService title="services.projectServices.question">
      <p
        class="text-text text-lg md:text-xl font-normal flex flex-col max-w-[1164px]"
      >
        <span class="block text-justify">
          {{ $t("services.projectServices.descriptionOne") }}
        </span>
        <br />
        <span class="block text-justify">
          {{ $t("services.projectServices.descriptionTwo") }}
        </span>
      </p>
    </WhatTheService>
    <DetailedServiceSection
      title="services.projectServices.details"
      :details="locale === 'ar' ? detailsAr : detailsEn"
    />
    <CTASection
      title="services.projectServices.cta"
      buttonO="اطلب عرض مالي"
